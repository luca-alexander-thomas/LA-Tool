...
import uuid
import json
from . import login_function as login_func
...
def login(request):
    ...
    if request.method == "POST":
        ...
        user = login_func.login(username, password, request)
...
def sync_user_groups(user, access_token):
    ...
    try:
        ...
        if graph_response.status_code != 200:
            ...
            print(
                f"Fehler beim Abrufen der Gruppen: {graph_response.status_code}")
        ...
        user.save()
        print(
            f"User {user.username} Gruppen synchronisiert: {[g.name for g in django_groups_to_assign]}")
...
def microsoft_callback(request):
    ...
    try:
        ...
        last_name = user_info.get('surname', '') or 'User'
